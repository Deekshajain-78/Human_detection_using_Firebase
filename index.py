...
FIREBASE_URL="https://aiml-internship-d1fc9-default-rtdb.firebaseio.com/"
...
def send_to_firebase(person_detected):
    ...
    try:
        data={
            "person_detection":1 if person_detected else 0,
...
while True:
    ...
    r=model(frame)[0]
    person_count=sum(int(b.cls[0]) ==0 and b.conf[0]>0.5 for b in r.boxes)
    frame=r.plot()
